    ...
    if(assume.cladogenetic == TRUE){
        ...
        if(length(f) == 3){
            ...
            if(length(freqs == 2)){
                ...
                samp.freq.tree <- Ntip(phy) / sum(table(data.new[,1])[as.numeric(names(hh))+1] / f)
            }else{
                samp.freq.tree <- Ntip(phy) / sum(table(data.new[,1])[as.numeric(names(hh))+1] / f)
            }
        }else{
            if(length(f) == Ntip(phy)){
                samp.freq.tree <- Ntip(phy) / sum(table(data.new[,1])[as.numeric(names(hh))+1] / mean(f))
            }else{
...
DownPassHiGeosse <- function(phy, cache, hidden.states, bad.likelihood=-10000000, condition.on.survival, root.type, root.p, get.phi=FALSE, node=NULL, state=NULL, ode.eps=0) {
	#Some preliminaries:
