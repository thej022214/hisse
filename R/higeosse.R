...
ParametersToPassHiGeoSSE <- function(phy, data, f, model.vec, hidden.states){
    ...
    if(hidden.states == "TEST"){
        states = matrix(0,Ntip(phy),15)
        for(i in 1:Ntip(phy)){
            if(data[i]==1){states[i,10]=1}
            if(data[i]==2){states[i,11]=1}
            if(data[i]==0){states[i,12]=1}
            if(data[i]==4){states[i,13]=1}
            if(data[i]==5){states[i,14]=1}
            if(data[i]==3){states[i,15]=1}
        }
    }
