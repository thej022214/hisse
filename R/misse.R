    ...
    for (batch_index in sequence(total.chunks)) { # So, if we can do parallel, we do it in chunks so all cores are busy
        ...
        if(batch_index<total.chunks) {
            if(stop.deltaAICc>min.deltaAICc.this.chunk) {
                print(paste0("Best delta AICc in this set of parallel runs was ", round(min.deltaAICc.this.chunk,2), " which is less than the cutoff to stop running (",stop.deltaAICc,"), so starting another set of parallel runs"))
            } else {
                print(paste0("Best delta AICc in this set of parallel runs was ", round(min.deltaAICc.this.chunk,2), " which is greater than the cutoff to stop running (",stop.deltaAICc,"), so stopping here"))
                break()
            }
        }
...
DevOptimizeMiSSE <- function(p, pars, dat.tab, gen, hidden.states, fixed.eps, nb.tip, nb.node, condition.on.survival, root.type, root.p, np, ode.eps, fossil.taxa, psi.type, fix.type, strat.cache) {
    ...
    runSilent <- function(branch.type) {
        ...
        if(branch.type == 3){
			...
			if(cache$psi.type == "m_only"){
				capture.output(res <- lsoda(yini, times, func = "misse_kcensor_derivs", pars, initfunc="initmod_misse", dllname = "hisse", rtol=1e-8, atol=1e-8))
			}else{
...
ParametersToPassMiSSE <- function(model.vec, hidden.states, fixed.eps, nb.tip, nb.node, psi.type, bad.likelihood, ode.eps){
