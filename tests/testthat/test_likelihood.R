...
test_that("GeoSSE_fGeoSSSE_test1", {
    ...
    phy$node.label <- NULL
    cache.slim <- hisse:::ParametersToPassGeoHiSSEfast(model.vec=model.vec, hidden.states=FALSE,nb.tip=Ntip(phy), nb.node=Nnode(phy), bad.likelihood=exp(-500), ode.eps=0)
    gen <- hisse:::FindGenerations(phy)
...
test_that("GeoSSE_fGeoSSSE_test2", {
    ...
    phy$node.label <- NULL
    cache.slim <- hisse:::ParametersToPassGeoHiSSEfast(model.vec=model.vec, hidden.states=TRUE, nb.tip=Ntip(phy), nb.node=Nnode(phy), bad.likelihood=exp(-500), ode.eps=0)
    gen <- hisse:::FindGenerations(phy)
