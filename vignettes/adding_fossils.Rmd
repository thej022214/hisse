...
vignette: >
   ...
   %\VignetteIndexEntry{Incorporating Fossils}
...
As of version 2.1.14, we have added the ability for HiSSE, MuHiSSE, and MiSSE to handle fossils as well as stratigraphic ranges. The fossilized birth-death model (FBD) follows that of Stadler (2010), and includes a new parameter, $\psi$, that estimates the rate of fossils preservation in a birth-death model (see Beaulieu and O'Meara 2023). We have also extended the canonical FBD model to deal with fossil sets that lack sampled ancestors (fossilized birth-death of terminal fossils only, FBDT), the details of which will be included in a forthcoming paper (Beaulieu and O'Meara, Submitted). Below I will demonstrate how to set up a MiSSE model when the tree and data contain fossils. I will also demonstrate a simple test to show that both the HiSSE and MuHiSSE likelihoods are correct.

Before getting started, there are two important terms that need to be clearly understood. Under the canonical FBD there are actually two types of fossils that are sampled. The first are referred to as *m* fossils, which represent extinct tips -- that is, they represent distinct branches that went extinct before the present. The second are referred *k* fossils, which represent samples of lineages that eventually made it to the present. In most real world cases, both fossil types must be present for the FBD to work well, though having *k* fossils is not absolutely critical for *hisse* whereas *m* fossils are. In cases where *k* fossils are absent `hisse` automatically defaults to the FBDT model.
...
Be sure to load the `hisse` and `TreeSim` packages. I will first simulate a tree with fossil samples under a simple fossilized birth-death model. This will be used as input into MiSSE, which sets up and executes a completely trait-free version of a HiSSE model (i.e., hidden states only). The first step is to simulate a tree that contains the complete history of the trees -- that is, includes both taxa that survived to the present and those that did not:
```{r, eval=TRUE}
set.seed(42)
...
```{r, eval=FALSE}
k.samples.hisse <- data.frame(taxon1="sp12", taxon2="sp12", timefrompresent=3.164384, 
          state=1, stringsAsFactors=FALSE)
head(k.samples.hisse)
```

and `MuHiSSE()`,
```{r, eval=FALSE}
k.samples.muhisse <- data.frame(taxon1="sp12", taxon2="sp12", timefrompresent=3.164384,
          state1=0, state2=1, stringsAsFactors=FALSE)
head(k.samples.muhisse)
```
...
Note that the only two additions to the function call are `includes.fossils=TRUE`, which indicates that the tree contains at least *m* fossils, and `k.samples=` that takes the table of edge samples. If no edge samples exist, this can be set to `NULL` and `hisse` will automatically default to the FBDT model:
```{r, eval=FALSE}
one.rate <- MiSSE(fbd.tree$phy, f=1, turnover=turnover, eps=eps, 
          includes.fossils=TRUE, k.samples=NULL, sann=TRUE, sann.its=1000)
```

In such cases, the estimate of $\psi$ will *only* reflect sampling of extinct tips. As we show in Beaulieu and O'Meara (2023), assuming both *m* and *k* fossils, but setting `k.samples=NULL` has a rather pronounced effect on the parameter estimates generally.
...
## A cautionary tale about empirical trees and what is considered 'surviving'

Trees containing fossil taxa are necessarily non-ultrametric. However, numerical differences in branch lengths can sometimes cause surviving tips to terminate slightly before the present, potentially resulting in their incorrect classification as extinct. We now provide a function called `CheckFossilTree()`, which allows users to check how tips will be classified by `hisse` and determine whether the default `extinct.tol` should be adjusted before fitting a fossil model. The extinct.tol argument is now also included in the function calls to `MiSSE()`, `hisse()`, and `MuHiSSE()`. 

As an example, here is Zamiaceae, pruned from a much larger empirical tree of cycads from Coiro et al. (2023) that includes many fossil tips. Within this subtree, there should be 11 extinct tips and 111 surviving tips. However, if we were to run `MiSSE()` using the default tolerance of `extinct.tol=.Machine$double.eps^.50`, we get the following:

```{r, eval=TRUE}
tree <- read.tree("zam.tre")
CheckFossilTree(tree, return.list=FALSE)
```

Yikes -- only 2 taxa are considered surviving here. If this check was never used and we ran `MiSSE()` we would probably notice that something is off because the estimate of $\psi$ hits the upper bound of 1. However, since we did we can triage before fitting the model. Setting `return.list=TRUE` returns the estimated ages of the tips classified as surviving and extinct, which can help determine whether the default tolerance is too strict. In this case, many taxa known to be surviving have very small positive tip ages due to numerical differences in branch lengths. Here, `extinct.tol=.Machine$double.eps^.25` is a more appropriate tolerance:
```{r, eval=TRUE}
tree <- read.tree("zam.tre")
CheckFossilTree(tree, extinct.tol=.Machine$double.eps^.25, return.list=FALSE)
```

This returns the expected numbers of 111 surviving and 11 extinct tips. **Note** -- if the tolerance is changed when calling `MiSSE()`, `hisse()`, or `MuHiSSE()`, the tolerance will also have to be adjusted reconstructing the states on the tree (e.g., `MarginReconMiSSE()`).
...
pp <- MuHiSSE(phy=fbd.tree$phy, data, f=c(1,1,1,1), turnover=c(1,1,1,1), 
            eps=c(1,1,1,1), trans.rate=trans.rate, includes.fossils=TRUE,
            k.samples=fbd.tree$k.samples)
...
cache <- hisse:::ParametersToPassfHiSSE(model.vec, hidden.states=hidden.states, 
            nb.tip=Ntip(phy.k), nb.node=Nnode(phy.k), psi.type="m+k", 
            bad.likelihood=-300, f=c(1,1), ode.eps=0)
cache$psi <- 0.01
...
cache = hisse:::ParametersToPassMiSSE(model.vec=model.vec, hidden.states=1, 
            ...
            psi.type="m+k", bad.likelihood=exp(-500), ode.eps=0)#
cache$psi <- 0.01
gen <- hisse:::FindGenerations(phy.k)
...
cache <- hisse:::ParametersToPassMuHiSSE(model.vec=model.vec, hidden.states=FALSE, 
            psi.type="m+k", nb.tip=Ntip(phy.k), nb.node=Nnode(phy.k), 
            bad.likelihood=exp(-500), f=c(1,1,1,1), ode.eps=0)
cache$psi <- 0.01
gen <- hisse:::FindGenerations(phy.k)
...
Beaulieu, J.M., and B.C. O'Meara. (Submitted). A fossilized birth-death model for fossil records lacking sampled ancestors. Submitted.

Beaulieu, J.M., and B.C. O'Meara. (2023). Fossils do not substantially improve, and may even hard, estimates of diversification rate heterogeneity. Systematic Biology, 72:50-61.
...
Coiro, M., R. Allio, N. Mazet, L.J. Seyfullah, and F.L. Condamine. (2023). Reconciling fossils with phylogenies reveals the origin and macroevolutionary processes explaining the global cycad biodiversity. New Phytology, 240:1616-1635.

Stadler, T. (2010). Sampling-through-time in birth-death trees. Journal of Theoretical Biology, 267:396-404.

Stadler T., A. Gavryushkina, R.C.M. Warnock, A.J. Drummond, and T.A. Heath. (2018). The fossilized birth-death model for analysis of stratigraphic range data under different speciation modes. Journal of Theoretical Biology 447:41-55.
