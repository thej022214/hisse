---
title: "Running hiGeoSSE"
author: "Caetano, D.S., O'Meara, B.C., and Beaulieu J.M."
...
vignette: >
   ...
   %\VignetteIndexEntry{Running hiGeoSSE}
   \usepackage[utf8]{inputenc}
---

## Getting started

This tutorial gives some basic information on how to set up and execute parameter estimates for a series of models using hiGeoSSE (Caetano et al., in review). The hiGeoSSE models can be used to infer ancestral ranges, rates of dispersion and extirpation as well as testing hypothesis about range-dependent diversification processes.

The main difference between GeoSSE and hiGeoSSE is that here we implement models that allow for diversification rate variation both within and between geographical areas. Such models are more adequate to empirical data then homogeneous diversification rates implied by GeoSSE (as well as BiSSE). The hiGeoSSE models belong to the same category of Hidden-Markov models as HiSSE. Thus, the concepts will be familiar to you if you have some experience with HiSSE (and vice-versa).
...
install_github(repo = "thej022214/hisse", ref = "higeosse_dev")
```
...
pars <- SimulateHiGeoSSE(hidden.areas = 1, return.HiGeoSSE_pars = TRUE)
pars
```
...
Now we can use the parameters with the same function we applied before `SimulateHiGeoSSE` to generate both the data and the phylogeny. 
...
```{r}
set.seed(1234)
sim.higeosse <- SimulateHiGeoSSE(pars=pars, hidden.areas = 1, x0 = "0A", max.taxa = 200)
```
...
speciation <- c(1,1,1)
extirpation <- c(1,1)
trans.rate <- TransMatMakerHiGeoSSE(hidden.areas=0)
# mod1 <- HiGeoSSE(phy = sim.higeosse$phy, data = sim.higeosse$data, f=c(1,1,1),
...
extirpation <- c(1,2)
trans.rate <- TransMatMakerHiGeoSSE(hidden.areas=0)
# mod2 <- HiGeoSSE(sim.higeosse$phy, data = sim.higeosse$data, f=c(1,1,1),
#                  speciation=speciation, extirpation=extirpation,
#                  hidden.areas=FALSE, trans.rate=trans.rate)
```
...
```{r}
## Model 3. HiGeoSSE model with 1 hidden area, no range-dependent diversification.
...
trans.rate <- TransMatMakerHiGeoSSE(hidden.areas=1)
# mod3 <- HiGeoSSE(sim.higeosse$phy, data = sim.higeosse$data, f=c(1,1,1),
#                  speciation=speciation, extirpation=extirpation,
#                  hidden.areas=TRUE, trans.rate=trans.rate)

## Model 4. HiGeoSSE model with 1 hidden area, no range-dependent diversification.
speciation <- c(1,2,3,4,5,6)
extirpation <- c(1,2,3,4)
trans.rate <- TransMatMakerHiGeoSSE(hidden.areas=1)
# mod4 <- HiGeoSSE(sim.higeosse$phy, data = sim.higeosse$data, f=c(1,1,1),
#                  speciation=speciation, extirpation=extirpation,
#                  hidden.areas=TRUE, trans.rate=trans.rate)
```
...
To compute model weight we can use one of the functions of the package. This will work with both HiSSE and HiGeoSSE objects.
```{r}
...
list.higeosse <- list(model1 = mod1, model2 = mod2, model3 = mod3, model4 = mod4)
GetModelWeight(list.higeosse)
```
...
load( "higeosse_recons_vignette.Rsave" )
```
...
plot.higeosse.states(x = recon.models, rate.param = "net.div", type = "phylogram", 
