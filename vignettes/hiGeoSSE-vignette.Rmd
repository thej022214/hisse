...
pars <- SimulateHiGeoSSE(hidden.areas = 2, return.HiGeoSSE_pars = TRUE)
pars
```
...
pars$model.pars[,3] <- c(0.4, 0.4, 0.4, 0.03, 0.03, 0.05, 0.05)
pars$q.AB[1,2] <- pars$q.AB[2,3] <- pars$q.AB[3,2] <- pars$q.AB[2,1] <- 0.05
pars$q.A[1,2] <- pars$q.A[2,3] <- pars$q.A[3,2] <- pars$q.A[2,1] <- 0.05
pars$q.B[1,2] <- pars$q.B[2,3] <- pars$q.B[3,2] <- pars$q.B[2,1] <- 0.05
pars
```
...
```{r}
sim.higeosse <- SimulateHiGeoSSE(pars=pars, hidden.areas = 2, x0 = "AB0", max.taxa = 200)
key.number <- c(0, 0, 0 , 1, 1, 1, 2, 2, 2)
key.name <- c("AB0","AB1", "AB2", "A0","A1", "A2", "B0","B1", "B2")
data <- lapply(sims, function(x) 
    sapply(x$data, function(y) key.number[match(y, key.name)])
    )
data <- lapply(data, function(x) cbind(names(x), unname(x) ) )
phy <- lapply(sims, function(x) x$phy)
```


save(phy, data, sims, file="sims_2_500_data.RData")
```
