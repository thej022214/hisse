...
To demonstrate, we summarized the ratios in diversification between observed states 0 and 1 across the tips for two scenario in which HiSSE performed poorly from a model rejection perspective. Specifically, 
we examined a true-SSD scenario (6)  that  
Points to make:
1) true-SSD scenario we do well, slightly underestimate the ratio, and in 4 data sets HiSSE returns the wrong ratio with respect to turnover. This is due to poor estimates of extinction, which inflated turnover rates. Relying on speciation only, however, all data sets return the proper ratio.
2) non-SSD is clearly centered on a ratio of 1, meaning no difference, despite the fact that 54% of the data sets were found to have strong evidence favoring a SSD interpretation. In fact, this scenario has an average of 3% upwards bias, with roughly 70% of all data sets falling within 10% in either direction.

```{r, eval=TRUE, echo=FALSE, fig.height=5, fig.width=7, fig.cap = "Model-averaged parameter estimates of turnover and speciation from two scenarios, a true SSD scenario (A,B) where our HiSSE model set showed high 'false negative' rates (i.e., failed to reject a trait-independent scenario), and a non-SSD scenario (C,D) which exhibited a 54% false-positive rate. In the case of the non-SSD scenario, it clearly shows that despite the poor performance of from a model rejection perspective, examining the the model parameters would indicate that on average, there are no differences in diversification rates among observed states."}
