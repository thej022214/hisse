...
## Acounting for tip fog

HiSSE allows uncertainty in the observed character states at the tips to be incorporated using the new `tip.fog` argument. Tip fog represents the probability that an observed state differs from the underlying state generated by the evolutionary process. This uncertainty can arise from measurement error, intraspecific variation, misclassification, or other sources, without requiring that its particular cause be specified. Ignore this uncertainty can bias estimates of transition and diversification rates and influence model selection (see Beaulieu and O'Meara 2025).

Tip fog can either be fixed by the user or estimated as part of the model. For a binary character, a single probability can be supplied:

```{r, eval=FALSE}
fit <- hisse(phy, data, turnover=c(1,2), eps=c(1,1), hidden.states=FALSE
             tip.fog=0.10)
```

Here, `tip.fog=0.10` assumes a 10% probability that the observed state of a species differs form it's underlying state. A single value is applied equally to both observe states. State-specific values can instead be supplied as a vector:
```{r, eval=FALSE}
tip.fog=c(0.05, 0.10)
```
which allows the amount of tip fog associated with observed states 0 and 1 to differ.

Tip fog can also be estimated directly from the data. In this case, `tip.fog` is supplied as a vector of integer parameter indices, in the same spirit as other paramter-index vectors used by `hisse`. For example,
```{r, eval=FALSE}
tip.fog=c(1,1)
```
estimates a single tip-fog probability shared by states 0 and 1, whereas:
```{r, eval=FALSE}
tip.fog=c(1,2)
```
estimates a separate probability for each observed state. Estimated tip-fog probabilities are constrained between approximately 0 and 0.5 and are returned in the fitted object as:
```{r, eval=FALSE}
fit$tip.fog.probs
```
Setting `tip.fog=NULL`, the default, assumes no tip fog and therefore treats the observed tip states as known without this additional source of uncertainty.
...
Beaulieu, J.M., and B.C. O'Meara. (2025). Navigating "tip fog": embracing uncertainty in tip measurements. Evolition. 79:1131-1142.
