...
##A common mistake
I wanted to highlight a common mistake I've seen through my email correspondence with several users of hisse. It may be of interest to test a model where the hidden state is associated with a single observed state. In other words, the model would contain state 0A, 1A, and 1B. The diversification parameters might look something like this:
```{r, eval=TRUE}
...
I've created two additional `hisse.states` objects that I will use to demonstrate this: one reconstruction is based on the null-two model [turnover.anc=c(1,1,2,2], and the other assumes four free turnover rates [i.e., turnover.anc=c(1,2,3,4)]; in all cases I assumed equal transition rates and equal extinction fractions. I recognize that there are many ways to generate a list. But here is one way, where I'm assuming that the marginal reconstructions from three models are saved to the directory we are working from:
```{r, eval=TRUE}
